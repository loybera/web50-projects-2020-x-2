$(document).ready(function() {
    ...
    $('div.chat_list').click(function() {
        ...
        $(".msg_history").hide();
        $(".msg_history#msg_history-"+channel).show();
    ...
    const in_msg_template = Handlebars.compile(document.querySelector('#in_message').innerHTML);
    function add_in_msg(channel, post, audit_data) {
        ...
        // Add post to DOM.
        document.querySelector('#msg_history-'+channel).innerHTML += msg;
    }
    const out_msg_template = Handlebars.compile(document.querySelector('#out_message').innerHTML);
  ...
  socket.on('my_room', function(msg, cb) {
    ...
    if (channel) {
        ...
        // $('#msgs-'+channel).append('<li>' + $('<div/>').text(   msg .data ).html());
...
//   }, 1000);
  ...
  $('form#join').submit(function(event) {
        $('div.modal-chat_list').click(function() {
            var channel = $(this).attr('id');
            ...
            $(this).addClass('active_chat');
            $("#room_name").val(channel);
            // $('#myChannelModal').modal('hide');
            $('.modal-backdrop').hide();
            // document.querySelector('#inbox_chat').innerHTML +=  $(this);
